...
public class TreeGravestoneCanvas : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        // F 키 입력은 GlobalCounter.TreeCounter가 1일 때만 활성화
        if (isPlayerNearby && !isGravestoneActivated && GlobalCounter.TreeCounter == 3 && Input.GetKeyDown(KeyCode.F))
        {
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            if (canvas != null)
            {
                canvas.enabled = true; // 언제나 플레이어가 접근하면 활성화
            }
            isPlayerNearby = true;
        }
    }
    ...
    // 비석 오브젝트 활성화 코루틴 (0.5초 딜레이 후 활성화)
        ...
        if (etcGroup != null)
        {
            ...
            if (gravestoneGroup != null)
            {
                ...
                if (rock01 != null && rock011 != null)
                {
                    ...
                    rock011.SetActive(true);
                    Debug.Log("나무 비석이 성공적으로 활성화되었습니다.");
                    canvas.enabled = false;

                    // 전역 변수 업데이트
                    GlobalCounter.IsTreeGravestoneActivated = true;
